...
> **交给 Agent：** 直接把 `https://github.com/shaun17/PageComet` 发给任意能读取仓库并操作终端的 Coding Agent，并告诉它“请按仓库里的 Agent 安装部署流程完成安装、配置、测试和发布”；它会执行可自动化步骤，只在 Notion、Cloudflare 授权和个人资料处向你索取必要输入。
...
| 目标 | 推荐入口 | 需要准备 |
| --- | --- | --- |
| 让 Agent 完成安装部署 | [Agent 自动安装部署](#agent-自动安装部署) | 一个能操作终端的 Agent；过程中提供 Notion 与 Cloudflare 授权 |
| 先看看项目是否适合自己 | [零密钥快速预览](#零密钥快速预览) | Node.js 22.13 或更高版本 |
| 自己从头配置生产站点 | [人工安装与部署](#人工安装与部署) | Node.js、Notion 工作区、Cloudflare 账户 |
| 已经使用旧版 PageComet | [从旧版本升级](#从旧版本升级) | 现有本机配置和两个 Notion 内容源 |

### Agent 自动安装部署

将下面这段话连同仓库链接发给 Agent 即可：

```text
请按照仓库 README 和 docs/agent-deploy.md，在我的环境中完成 PageComet 的安装、配置、测试与 Cloudflare Pages 发布。不要把任何密钥或个人配置提交到 Git；需要 Notion、Cloudflare 登录或个人资料时，只向我索取当前步骤必需的信息。完成后验证正式地址，并汇报测试、构建、部署和线上检查结果。
```

Agent 的完整执行顺序、暂停条件和完成标准见 [`docs/agent-deploy.md`](./docs/agent-deploy.md)。README 仍是配置字段、Notion schema 和安全规则的唯一事实来源；Agent 指南只负责把这些步骤按可执行顺序串起来。

Agent 可以自动完成仓库克隆、依赖安装、本机文件初始化、配置校验、测试、构建、Cloudflare Pages 创建与发布。以下步骤可能需要你亲自授权或提供信息：

- 登录自己的 Notion 和 Cloudflare 账户；
- 确认站点名称、域名、简介和联系方式；
- 创建或确认两个 Notion Data Source、私有 Form 与只读 Integration；
- 授权 Cloudflare OAuth，或以安全方式提供 API Token；
- 添加自定义域名和修改 DNS。

这些是账户所有权边界，不应通过把密码或密钥粘贴到公开聊天中绕过。没有浏览器或账户操作能力的 Agent 会在对应步骤停下并给出最小人工操作说明，完成授权后继续执行。

### 零密钥快速预览

仓库提供 `.nvmrc` 和完整 fixture，不需要 `.env`、Notion 或 Cloudflare 账号：
...
打开终端显示的本地地址即可。`dev:fixture` 使用仓库内的文章、流水账和媒体夹具，不读取 `.env`，也不访问 Notion。
...
这里的“即开即用”分为两层：工程和 fixture 可以零配置启动；真实生产内容采用引导式配置，需要在自己的 Notion 工作区建立两套 schema 与 Form。仓库目前不附带可复制的 Notion 模板，也不会在未授权时改动使用者的工作区。
...
下面是人类可以逐步执行、Agent 也可以直接遵循的生产配置主线。开始前需要：

- Git；
- Node.js 22.13 或更高版本；
- 可以创建 Integration 与数据库的 Notion 工作区；
- 一个 Cloudflare 账户。
...
此时 `npm test` 只使用虚构配置和 fixture，用来确认本机环境与项目基线正常。

### 1. 创建本机站点配置

复制示例，再编辑被 Git 忽略的本机文件：
...
首次安装可以跳过本节。旧版文章型流水账需要把正文映射到新属性，不能只移动 Page：
...
4. 保留 Integration Token，下一步只写入本机 `.env`。
...
本机交互式部署可以把 Cloudflare Token 与账户 ID 留空，稍后使用 Wrangler OAuth。CI 或无浏览器环境必须安全提供 `CLOUDFLARE_API_TOKEN` 和 `CLOUDFLARE_ACCOUNT_ID`，不要把真实值写进仓库文件。
...
至少检查首页四个入口、流水账列表和一篇文章详情。正式输出位于 `dist/`，此时访客侧已经不再依赖 Notion。
...
部署脚本不会把 Notion Token、两个 Data Source ID 或其他未知环境变量传给 Wrangler。Notion 与 Cloudflare 凭据的原值及常见编码表示都会参与最终产物扫描。任一步骤失败都会中止，不会上传未通过检查的站点。
...
| 命令 | 用途 | 是否需要个人配置或密钥 |
| --- | --- | --- |
| `npm run dev:fixture` | 使用固定文章和流水账内容离线开发 | 否 |
| `npm run build:fixture` | 生成可重复的测试站点 | 否 |
| `npm run check` | Astro 与 TypeScript 类型检查 | 否 |
| `npm test` | 类型检查、fixture 构建、完整测试和产物校验 | 否 |
| `npm run dev` | 从两个 Notion Data Source 读取真实内容并启动 Astro | 是 |
| `npm run validate:site-config` | 验证本机个人配置存在且结构有效 | 是 |
| `npm run build` | 从 Notion 生成生产 `dist/` | 是 |
| `npm run verify:dist` | 检查文件大小、哈希、引用、临时地址和凭据 | 构建后执行 |
| `npm run preview` | 本地预览当前 `dist/` | 构建后执行 |
| `npm run pages:dev` | 构建真实内容并用 Wrangler 本地运行 Pages | 是 |
| `npm run cloudflare:login` | 使用浏览器登录 Cloudflare | Cloudflare 账户 |
| `npm run pages:create` | 按 `.env` 创建 Direct Upload 项目，只需执行一次 | 是 |
| `npm run deploy` | 测试、真实构建、安全校验并上传生产站点 | 是 |

fixture、类型检查和 GitHub Actions 在没有个人配置时使用 Alice 示例，因此 Fork 后无需任何 Secret 即可验证工程。`npm run dev`、`npm run build` 和 `npm run deploy` 会检查本机 `site.config.mjs`。
...
├── docs/
│   └── agent-deploy.md          # Agent 自动安装部署执行清单
├── src/
